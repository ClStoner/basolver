...
{
  public:
    POINTER_TYPEDEFS(IntegrationBase);
          ...
          sum_dt{0.0}, delta_p{Eigen::Vector3d::Zero()}, delta_q{Eigen::Quaterniond::Identity()}, delta_v{Eigen::Vector3d::Zero()}
    {
        ...
        noise.block<3, 3>(0, 0) = (params.accN * params.accN) * Eigen::Matrix3d::Identity();
        noise.block<3, 3>(3, 3) = (params.gyrN * params.gyrN) * Eigen::Matrix3d::Identity();
        noise.block<3, 3>(6, 6) = (params.accN * params.accN) * Eigen::Matrix3d::Identity();
        noise.block<3, 3>(9, 9) = (params.gyrN * params.gyrN) * Eigen::Matrix3d::Identity();
        noise.block<3, 3>(12, 12) = (params.accW * params.accW) * Eigen::Matrix3d::Identity();
        noise.block<3, 3>(15, 15) = (params.gyrW * params.gyrW) * Eigen::Matrix3d::Identity();
    }
    ...
    void midPointIntegration(double _dt, const Eigen::Vector3d &_acc_0, const Eigen::Vector3d &_gyr_0, const Eigen::Vector3d &_acc_1,
                             const Eigen::Vector3d &_gyr_1, const Eigen::Vector3d &delta_p, const Eigen::Quaterniond &delta_q,
    ...
    void propagate(double _dt, const Eigen::Vector3d &_acc_1, const Eigen::Vector3d &_gyr_1)
    {
        ...
        midPointIntegration(_dt, acc_0, gyr_0, _acc_1, _gyr_1, delta_p, delta_q, delta_v, linearized_ba, linearized_bg, result_delta_p,
                            result_delta_q, result_delta_v, result_linearized_ba, result_linearized_bg, 1);
                                          ...
                                          const Eigen::Vector3d &Bgj)
    {
        ...
        residuals.block<3, 1>(O_P, 0) = Qi.inverse() * (0.5 * params.G * sum_dt * sum_dt + Pj - Pi - Vi * sum_dt) - corrected_delta_p;
        residuals.block<3, 1>(O_R, 0) = 2 * (corrected_delta_q.inverse() * (Qi.inverse() * Qj)).vec();
        residuals.block<3, 1>(O_V, 0) = Qi.inverse() * (params.G * sum_dt + Vj - Vi) - corrected_delta_v;
