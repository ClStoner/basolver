...
int PoseGraph::detectLoop(KeyFrame *keyframe, int frame_index)
{
    ...
    if (DEBUG_IMAGE)
    {
        for (unsigned int i = 0; i < ret.size(); i++)
        {
            ...
            putText(tmp_image, "index:  " + to_string(tmp_index) + "loop score:" + to_string(ret[i].Score), cv::Point2f(10, 50),
...
void PoseGraph::optimize4DoF()
{
    while (true)
    {
        ...
        if (cur_index != -1)
        {
            ...
            for (it = keyframelist.begin(); it != keyframelist.end(); it++)
            {
                ...
                for (int j = 1; j < 5; j++)
                {
                    if (i - j >= 0 && sequence_array[i] == sequence_array[i - j])
                    {
                        ...
                        ceres::CostFunction *cost_function = FourDOFError::Create(relative_t.x(), relative_t.y(), relative_t.z(), relative_yaw,
                ...
                if ((*it)->has_loop)
                {
                    ...
                    ceres::CostFunction *cost_function = FourDOFWeightError::Create(relative_t.x(), relative_t.y(), relative_t.z(), relative_yaw,
                                                                                    euler_conncected.y(), euler_conncected.z());
                    problem.AddResidualBlock(cost_function, loss_function, euler_array[connected_index], t_array[connected_index], euler_array[i],
                                             t_array[i]);
                }
            ...
            std::ofstream fout("/home/cl/workspace/project/intel/intel_stereo/src/intel_stereo/output/pg.txt", std::ios::app);
            fout << keyframelist.size() << " " << time_solve << std::endl;
            fout.close();
            static double avg_time = 0;
            static double cnt_solve = 0;
            avg_time += time_solve;
            cnt_solve += 1.0;
            std::cout << "avg_pg : " << avg_time / cnt_solve << std::endl;
            ...
            for (it = keyframelist.begin(); it != keyframelist.end(); it++)
            {
                ...
                for (int j = 1; j < 5; j++)
                {
                    if (i - j >= 0 && sequence_array[i] == sequence_array[i - j])
                    {
                        ...
                        ceres::CostFunction *vo_function = RelativeRTError::Create(relative_t.x(), relative_t.y(), relative_t.z(), relative_q.w(),
                ...
                if ((*it)->has_loop)
                {
                    ...
                    ceres::CostFunction *loop_function = RelativeRTError::Create(relative_t.x(), relative_t.y(), relative_t.z(), relative_q.w(),
                                                                                 relative_q.x(), relative_q.y(), relative_q.z(), 0.1, 0.01);
                    problem.AddResidualBlock(loop_function, loss_function, q_array[connected_index], t_array[connected_index], q_array[i],
                                             t_array[i]);
                }
...
void PoseGraph::savePoseGraph()
{
    ...
    for (it = keyframelist.begin(); it != keyframelist.end(); it++)
    {
        ...
        fprintf(pFile, " %d %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %d %f %f %f %f %f %f %f %f %d\n", (*it)->index, (*it)->time_stamp,
                VIO_tmp_T.x(), VIO_tmp_T.y(), VIO_tmp_T.z(), PG_tmp_T.x(), PG_tmp_T.y(), PG_tmp_T.z(), VIO_tmp_Q.w(), VIO_tmp_Q.x(), VIO_tmp_Q.y(),
                VIO_tmp_Q.z(), PG_tmp_Q.w(), PG_tmp_Q.x(), PG_tmp_Q.y(), PG_tmp_Q.z(), (*it)->loop_index, (*it)->loop_info(0), (*it)->loop_info(1),
                (*it)->loop_info(2), (*it)->loop_info(3), (*it)->loop_info(4), (*it)->loop_info(5), (*it)->loop_info(6), (*it)->loop_info(7),
        ...
        for (int i = 0; i < (int)(*it)->keypoints.size(); i++)
        {
            ...
            fprintf(keypoints_file, "%f %f %f %f\n", (*it)->keypoints[i].pt.x, (*it)->keypoints[i].pt.y, (*it)->keypoints_norm[i].pt.x,
...
void PoseGraph::loadPoseGraph()
{
    ...
    while (fscanf(pFile, "%d %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %d %lf %lf %lf %lf %lf %lf %lf %lf %d", &index, &time_stamp,
                  &VIO_Tx, &VIO_Ty, &VIO_Tz, &PG_Tx, &PG_Ty, &PG_Tz, &VIO_Qw, &VIO_Qx, &VIO_Qy, &VIO_Qz, &PG_Qw, &PG_Qx, &PG_Qy, &PG_Qz, &loop_index,
                  &loop_info_0, &loop_info_1, &loop_info_2, &loop_info_3, &loop_info_4, &loop_info_5, &loop_info_6, &loop_info_7,
                  &keypoints_num) != EOF)
    {
        /*
        ...
        */
        ...
        KeyFrame *keyframe =
            new KeyFrame(time_stamp, index, VIO_T, VIO_R, PG_T, PG_R, image, loop_index, loop_info, keypoints, keypoints_norm, brief_descriptors);
